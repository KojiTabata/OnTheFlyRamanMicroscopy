...
# lambda ran, grid, arm:
class Grids:
    def __init__(self, im, gs, type_of_cell, no, rounding=False):
        #self.w = w = im.shape[0]
        #self.h = h = im.shape[1]
    ...
    def mus(self):
        ...
        for i in range(self.K):
            l, t = self.arm_left_top(i)
            # v = np.mean(self.im[l:l+self.gs, t:t+self.gs, 2] > 0.5)
            if self.rounding:
                v = np.mean(self.im[l:l+self.gs, t:t+self.gs, 2] > 0.5)
            else:
    ...
    def arm_left_top(self, i):
        ...
        gh = (self.h // self.gs)
        gy, gx = i // gh, i % gh
        return (gw - gy - 1)*self.gs, gx*self.gs
        """
        # self.w = 240, self.h = 400
        gw = (self.w // self.gs)
        gh = (self.h // self.gs)
        # gx, gy = self.w - (i // gh + 1) * self.gs, (i % gh) * self.gs
        gx, gy = self.w - (i // gh + 1) * self.gs, (i % gh) * self.gs

        # print(i, self.w, self.h, gx, gy, self.gs)
    ...
    def evaluate(self, i, pt):
        ...
        # return 1.0 if self.im[pt[0], pt[1], 2] > 0.5 else 0.0
        if self.rounding:
            return 1.0 if np.argmax(self.im[pt[0], pt[1], :]) == 2 else 0.0
        else:
            # print(i, pt)
...
def run(delta, threshold, gs):
    ...
    for (type_of_cell, no), im in anom_data.items():
        ...
        for policy_name, policy in policies.items():
            outdir = f"result_t{threshold:.2f}_g{gs}_d{delta:.6f}_{policy_name}"
